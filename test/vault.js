...
describe('VaultContract', () => {
  ...
  let _FNCToken;
  let _VaultContract;
  let owner, delegator1, someone;

  beforeEach(async () => {
    [owner, delegator1, someone] = await ethers.getSigners();
  ...
  it('Fail: Set staking role as zero address', async () => {
    await expect(
      _VaultContract.connect(owner).setupStakingRole(ethers.constants.AddressZero)
    ).to.be.revertedWith("Vault: StakingContract is zero address");
  })

  it('Set staking role', async () => {
    // Success once
    await expect(
      _VaultContract.connect(owner).setupStakingRole(someone.address)
    ).not.to.be.reverted;

    // Fail after initialized
    await expect(
      _VaultContract.connect(owner).setupStakingRole(someone.address)
    ).to.be.revertedWith("Vault: StakingContract already initialized");
  })

  context('After initialized', async() => {
    beforeEach(async () => {
      await _VaultContract.connect(owner).setupStakingRole(owner.address);
    });
